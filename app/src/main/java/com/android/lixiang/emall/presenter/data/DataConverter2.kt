package com.android.lixiang.emall.presenter.data
...
class DataConverter2 {
    ...
    var recommandData: MutableList<HorizontalEntity>? = mutableListOf()
    ...
    fun recommandConvert(res: List<HomePageUnitsBean.DataBean.PiecesBean>): ArrayList<HomeMultiItemEntity> {
        ...
        for (i in 0 until size) {
            ...
            recommandData!!.add(HorizontalEntity(type, imageUrl, detail, posMark, title, position, productId, price))
        }
        val entity = HomeMultiItemEntity()
        entity.MULTIPLE_FIELDS["RECOMMAND"] = recommandData!!
        entity.MULTIPLE_FIELDS["SPAN_SIZE"] = 2
        entity.MULTIPLE_FIELDS["ITEM_TYPE"] = 4
        entities.add(entity)
        return entities
    }
}